...
async function render(slotAction: SlotAction): Promise<void> {
	const article = getCurrentArticle("giveaway");
...
/**
 * Shows whichever listing currently resolves as "the giveaway article" (see overlay.js's
 * resolveListingForType(true)) — the same one Start Giveaway/Draw Giveaway Winner implicitly act
 * on when no listing is explicitly selected in the overlay bar. Pressing cycles to the next
 * giveaway-type listing, wrapping around (lands back on itself if there's only one) — mirrors this
 * key's auction counterpart, {@link CurrentAuctionArticleDisplay}.
 */
@action({ UUID: "com.lkathke.whatnot-controller.current-giveaway-article-display" })
export class CurrentGiveawayArticleDisplay extends SingletonAction<Record<string, never>> {
	...
	override async onKeyDown(ev: KeyDownEvent<Record<string, never>>): Promise<void> {
		...
		const sentTo = broadcastCommand({ type: "cycleArticle", kind: "giveaway" });
